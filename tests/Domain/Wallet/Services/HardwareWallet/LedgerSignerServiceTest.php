...
/**
 * Unit tests for LedgerSignerService.
 *
 * Tests Ledger hardware wallet signing operations including
 * transaction preparation, signature validation, and derivation paths.
 */
class LedgerSignerServiceTest extends TestCase
{
    private LedgerSignerService $service;

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = new LedgerSignerService();
    }

    #[Test]
    public function it_returns_ledger_type(): void
    {
        $this->assertEquals('hardware_ledger', $this->service->getType());
    }
    ...
    #[Test]
    public function it_returns_correct_derivation_path_for_polygon(): void
    {
        $path = $this->service->getDerivationPath('polygon');
        $this->assertEquals("m/44'/60'/0'/0/0", $path);
    }
    ...
    {
        $path = $this->service->getDerivationPath('ethereum', 5);
        $this->assertEquals("m/44'/60'/0'/0/5", $path);
    }
    ...
    public function it_prepares_ethereum_transaction_for_signing(): void
    {
        ...
        $this->assertEquals('rlp', $prepared['encoding']);
        $this->assertStringStartsWith('0x', $prepared['raw_data']);

        // Check display_data has expected fields
